...
module.exports = async function(repositoryName, noOfDaysForFilterComments, paginationPerPageCount) {
    ...
    contributorsCommentsListAPIHttpMethodArray.push(contributorsCommitsListAPIHttpMethod)
    const contributorsCommentsAndCommitListArray = 
        await makeAsyncAPIConfig.makeMultipleAsyncAPIs(contributorsCommentsListAPIUrlArray, contributorsCommentsListAPIHttpMethodArray)
    ...
    contributorsCommentsAndCommitListArray.forEach((data) => {
        ...
        if (
            ...
            if (
                ...
                contributorsCommentsAndCommitListObject[contributorsIdListArray.indexOf(data.author.id)][
                    data.author.id
                ].commentCount += 1
                if (
                    contributorsCommentsAndCommitListObject[contributorsIdListArray.indexOf(data.author.id)][
        ...
        if (
            ...
            if (
                ...
                contributorsCommentsAndCommitListObject[contributorsIdListArray.indexOf(data.user.id)][
                    data.user.id
                ].commentCount += 1
                if (
                    contributorsCommentsAndCommitListObject[contributorsIdListArray.indexOf(data.user.id)][
