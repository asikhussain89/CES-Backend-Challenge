...
module.exports = async function() {
  const githubRateLimitData = await makeAsyncAPIConfig.makeSingleAsyncAPIs(
    '/rate_limit',
    'GET',
  )
  if (
    ...
    commonConsoleMessageLayout(
      `Error : ${githubRateLimitData[0].message}`,
      'red',
    )
    return false
  } else if (
    ...
    if (githubRateLimitData.data.rate.remaining === 0) {
      commonConsoleMessageLayout(
        "Rate Limit Crossed : You had already cross the Github API's rate limit",
        'red',
      )
      return false
    } else {
      commonConsoleMessageLayout(
        `Rate Limit Remaining : You have ${githubRateLimitData.data.rate.remaining} remaining Github API's rate limit`,
        'green',
      )
      return true
    }
